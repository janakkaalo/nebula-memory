...
function makeFaceTexture(symbol, active, matched) {
  ...
  // base
  const g = ctx.createRadialGradient(S * 0.35, S * 0.3, 40, S / 2, S / 2, S * 0.75)
  ...
  ctx.lineWidth = matched || active ? 14 : 8
  ctx.shadowColor = matched ? '#fcd34d' : '#67e8f9'
  ctx.shadowBlur = matched ? 46 : active ? 36 : 18
  roundRect(ctx, 26, 26, S - 52, S - 52, 36)
  ctx.stroke()
  ctx.shadowBlur = 0
  // center rune glow
  ...
  ctx.fillText(active || matched ? symbol : '✦', S / 2, S / 2 + 18)
  ctx.shadowBlur = 0
  // corner sparks
  ctx.fillStyle = 'rgba(255,255,255,.5)'
  ;[[70, 70], [S - 70, 70], [70, S - 70], [S - 70, S - 70]].forEach(([x, y]) => {
...
/**
 * AAA 3D memory: 16 physical cubes on a glowing disc.
 * Drag to orbit, scroll/pinch to zoom, tap to flip. Raycast picking.
 */
export default function CubeGame({ onWin, play }) {
  ...
  useEffect(() => {
    ...
    const camera = new THREE.PerspectiveCamera(W() < 560 ? 56 : 46, W() / H(), 0.1, 120)
    ...
    // ── ground disc + grid ──
    const discGeo = new THREE.CircleGeometry(7.4, 64)
    ...
    grid.position.y = -2.33
    grid.material.transparent = true
    grid.material.opacity = 0.28
    scene.add(grid)
    // halo ring
    const ringGeo = new THREE.RingGeometry(7.45, 7.7, 96)
    const ringMat = new THREE.MeshBasicMaterial({ color: 0x67e8f9, transparent: true, opacity: 0.5, side: THREE.DoubleSide })
    ...
    order.forEach((symbol, idx) => {
      ...
      mesh.position.set(
        (col - 1.5) * 1.68,
        (1.5 - row) * 1.62 + Math.sin(idx * 1.3) * 0.07,
        Math.cos(idx * 0.7) * 0.32
      )
    ...
    const starsFar = makeStars(340, [34, 20], 0.07, 0xbfdbfe, 0.8)
    const starsNear = makeStars(160, [26, 16], 0.13, 0x67e8f9, 0.9)
    ...
    function refreshTexture(mesh) {
      ...
      if (mesh.material[4].emissiveMap) mesh.material[4].emissiveMap.dispose()
      mesh.material[4].map = t
      mesh.material[4].emissiveMap = t
      mesh.material[4].emissive.set(m ? 0xfcd34d : o ? 0x67e8f9 : 0x0b1030)
      mesh.material[4].emissiveIntensity = m ? 0.85 : o ? 0.6 : 0.42
    ...
    function onUp(e) {
      ...
      dragging = false
      lastInteract = performance.now()
      // pinch-less wheel zoom handled separately; tap threshold
      if (moved > 9 || lock) return
      setPtr(e)
      const mesh = pick()
    ...
    function onWheel(e) {
      e.preventDefault()
      tDist = Math.max(7.5, Math.min(16, tDist + e.deltaY * 0.01))
      lastInteract = performance.now()
    }
    ...
    function onResize() {
      renderer.setSize(W(), H(), false)
      camera.aspect = W() / H()
      if (W() < 560) camera.fov = 58
      camera.updateProjectionMatrix()
    }
    window.addEventListener('resize', onResize)

    const clock = new THREE.Clock()
    const introFrom = { z: 15.5, y: 8.5 }
    let intro = 0
    function tick() {
      if (!alive) return
      ...
      if (intro < 1) {
        ...
        camera.position.z = introFrom.z + (dist - introFrom.z) * e
        camera.position.y = introFrom.y + (4.6 - introFrom.y) * e
      }
      rotX += (tRotX - rotX) * 0.09
      rotY += (tRotY - rotY) * 0.09
      dist += (tDist - dist) * 0.09
      group.rotation.set(rotX, rotY, 0)
      if (intro >= 1) {
        camera.position.z += (dist - camera.position.z) * 0.08
        camera.position.y += (4.6 - camera.position.y) * 0.06
      }
  ...
  useEffect(() => {
    if (won) {
      ...
      onWin({ score: Math.max(150, 1100 - moves * 12 - secs * 2), moves, time: secs })
    }
  ...
  return (
    <div className="cube-wrap">
      ...
      <div className="controls-row">
        <span className="cube-hint">Drag to orbit · Scroll / pinch to zoom · Tap two cubes to match runes</span>
        <button
